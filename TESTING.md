...
2136 unit and integration tests, plus 205 Playwright browser tests.
...
- `tests/e2e/productionPath.spec.ts`: title, New Game, hangar within 20 s of launch, hunt board, loadout, deployment card with a radio caption, Skip, the action HUD within 60 s with the creature named and four ability icons, drive and swing, Escape, Abort the hunt, rewards with at least eight lines, Return to hangar, no console errors; plus a 1366 by 768 pass over the hangar, hunt board and loadout with no horizontal scroll.
- `tests/e2e/huntFight.spec.ts`: ninety seconds of play (sprint in, chain, heavy, dodge, guard, plasma caster), the HUD sampled every pass. Asserts control inside 60 s, damage both ways and the director talking. Prints a summary line and writes `docs/screenshots/fmkh-final/`. Last run: control at 16.9 s, first contact at 25.0 s, Knifehead 100 to 77 percent and the machine 100 to 80 percent in ninety seconds, phase Signature reached, best combo 4.
