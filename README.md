...
- One loop, built for immediacy: title with the live bay behind it, the hangar with your Jaeger, a hunt board, a loadout sheet, a six second deployment card, arrival on the Anchorage quay with Knifehead a hundred and forty metres inland, a boss fight built on breaking the creature (armour, stability, vital damage, Drift Flow, grapples into the harbour, a clash, the Synchronized Breaker), a rewards sheet, and back to the hangar. A player build shows this path and nothing else; the first controllable machine arrives inside twenty seconds of launch, and `?hunt=knifehead` on the dev server boots straight into the fight
- A camera director with real states (free, soft framing, hard lock, sprint, close pressure, aim, grapple, clash, knockdown, finisher, boundary) that never rolls the horizon, sphere-casts against the city and answers the mouse in the same frame
- One orientation contract for every body (docs/ORIENTATION.md), enforced by a validator and a test scene, so the machine faces where it walks and never sinks or flips
