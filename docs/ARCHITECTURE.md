...
The loop a player sees, in `bootstrap.ts` under the heading "The production path". `production` is true in a player build and on the dev server with `?production=1`. The path reuses the existing states: the title is `MainMenu`, the dome is `Shatterdome`, and command, briefing, bay, deployment, fight and results are stages of `WorldMap` (`opStage`), because the globe and the ground view already live there. `Shatterdome` and `WorldMap` may now transition to each other.

- `showOpAlert` runs `ensureIncident`, which moves the clock in six-hour steps until a benchmark district (Hong Kong, Tokyo, Sydney, Manila) has a live breach, then raises the alert band and tells the interior crew.
- `enterCommand`, `enterBriefing`, `enterBay` render `src/ui/opScreens.ts` over the globe (the bay borrows the title composition for its live machine). Data comes from the incident, the deployment plan and the readiness report the world map already computed.
- `beginDeployment` calls the existing `deployTo`, then shows the cinematic; `enterFight` skips the carrier and lets `beginSortieOnTheGround` open the ground view. In production the machine steps out 3.2 s later on the district's waterfront (`layout.seawardBearingRadians`, 0.42 of the city radius), the creature spawns 460 m inland, and `dressDistrict` stands kit buildings on the nearest blocks.
- `updateOpFrame` runs every frame: hides the world and pilot panels in production and feeds `HudScreen` from the arena snapshot and the damage state. `advanceEncounter` runs every combat tick: `EncounterDirector` names the phase; cues become objective, warning, prompt, radio line, music boss phase, and the disruption (alert to attack, floor impulse, blast).
