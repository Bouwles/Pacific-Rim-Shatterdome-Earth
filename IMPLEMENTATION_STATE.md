...
- **Titan Break (2026-08-30).** The flagship fight rebuilt: orientation contract and validator (`docs/ORIENTATION.md`, `src/engine/orientation.ts`, `?scene=orientation`), both rigs rebuilt to it, the camera director (`src/jaegers/cameraDirector.ts`), Titan Break (`src/combat/titanBreak.ts`), Knifehead's controller (`src/combat/bossController.ts`), fourteen new moves, the harbour set (`src/engine/harborSet.ts`), the HUD (`src/ui/actionHud.ts`), the training simulation, the clash and the Synchronized Breaker in the bootstrap. Evidence and gate results in PROMPT_42_TITAN_BREAK_STATE.md; screenshots in `docs/screenshots/titan-break/`; a representative clear recorded by `tests/e2e/titanClear.spec.ts`.
- **FMKH rebuild (2026-08-29).** The player path is a hunt loop: hangar (`enterHangar`), hunt board (`src/data/hunts.ts`, `enterHunts`), loadout, a 6.5 s deployment card, arrival on the waterfront with the creature 140 m inland, the fight under `src/ui/actionHud.ts` with the action layout in `src/engine/pilotInput.ts`, rewards, records. Screens in `src/ui/hangarScreens.ts`, styles in `src/ui/theme-action.css`, state and gate results in FMKH_REBUILD_STATE.md, screenshots in `docs/screenshots/fmkh-final/`. Verified in the production build and by `tests/e2e/productionPath.spec.ts` and `tests/e2e/huntFight.spec.ts`. The on-foot Shatterdome, the globe command screen and the world panel remain in debug builds only.
